...
func (s *Store) CreateBranch(scriptID, name, fromSnapshotID string) (*Branch, error) {
	b := &Branch{
		ID:        uuid.New().String(),
		ScriptID:  scriptID,
		Name:      name,
	...
	_, err := s.db.Exec(
		`INSERT INTO branches (id, script_id, name, tip_id, created_at)
		 VALUES ($1, $2, $3, $4, $5)`,
		b.ID, b.ScriptID, b.Name, b.TipID, b.CreatedAt,
	)
	if err != nil {
...
func (s *Store) Commit(scriptID, branchID, content, message, authorID string) (*Snapshot, error) {
	...
	_, err = tx.Exec(
		`INSERT INTO snapshots
		 (id, branch_id, hash, content, message, author_id, parent_id, created_at)
		 VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
		snap.ID, snap.BranchID, snap.Hash, snap.Content, snap.Message, snap.AuthorID, snap.ParentID, snap.CreatedAt,
	)
	if err != nil {
...
func (s *Store) GetSnapshot(id string) (*Snapshot, error) {
	snap := &Snapshot{}
	err := s.db.QueryRow(
		`SELECT id, branch_id, hash, content, message, author_id, parent_id, created_at
		 FROM snapshots WHERE id = $1`, id,
	).Scan(
		&snap.ID, &snap.BranchID, &snap.Hash,
